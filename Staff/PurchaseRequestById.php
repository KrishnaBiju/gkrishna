...
$id = $_GET['uid'];
$qry = mysqli_query($conn, "SELECT DISTINCT u.* FROM `cart` c,`product` p,`user` u WHERE c.`uid`=u.`uid` AND c.`pid`=p.`pid` AND c.`status`='Paid' AND u.`uid`='$id'");
...
?>
                ...
                <div class="row">
                    <div class="col-md-10 mx-auto">
                        ...
                        <div class="owl-two owl-carousel owl-theme">
                            ...
                            <?php
                            include '../dbconnection.php';
                            $qry = mysqli_query($conn, "SELECT DISTINCT p.*,c.* FROM `cart` c,`product` p,`user` u WHERE c.`uid`=u.`uid` AND c.`pid`=p.`pid` AND c.`status`='Paid' AND u.`uid`='$id'");
                            if (mysqli_fetch_array($qry) > 0) {
                                $qry = mysqli_query($conn, "SELECT DISTINCT p.*,c.* FROM `cart` c,`product` p,`user` u WHERE c.`uid`=u.`uid` AND c.`pid`=p.`pid` AND c.`status`='Paid' AND u.`uid`='$id'");
        ...
        <section class="w3l-servicesblock1 py-1" id="services">
            <div class="container py-md-5 py-4">
                ...
                <?php
                $qry1 = mysqli_query($conn, "SELECT SUM(`total`) AS gtotal FROM `cart` WHERE `status`='Paid' and uid='$id'");
                ...
                ?>
                    ...
                    <div class="w3-services-grids py-lg-4">
                        ...
                        <div class="text-center mt-3">
                            <a href="../Action/AdminMarkAsDelivered.php" class="btn btn-style">Mark as Delivered</a>
                        </div>
                    </div>
